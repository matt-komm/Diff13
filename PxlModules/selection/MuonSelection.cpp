    ...
    public pxl::Module
{
    ...
    public:
	  ...
	  if ( i > -1 )
        ...
        {
	  double R04PFsumChargedHadronPt = particle->getUserRecord("R04PFsumChargedHadronPt").toFloat();
	  double R04sumNeutralHadronEt = particle->getUserRecord("R04PFsumNeutralHadronEt").toFloat(); //Correct it!
	  double R04PFsumPhotonEt = particle->getUserRecord("R04PFsumPhotonEt").toFloat(); //Correct it!
	  double R04PFsumPUPt = particle->getUserRecord("R04PFsumPUPt").toFloat();
	  double pT =  particle->getPt();
	  if( pT == 0 )
	    {
	      throw "Division by zero pT!";
	    }
	  return  (R04PFsumChargedHadronPt + std::max(R04sumNeutralHadronEt 
						      + R04PFsumPhotonEt
						      - _pfRelIsoCorDbBetatMuon*R04PFsumPUPt, 0.0)) / pT;
	}
